  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(children: [
      ...
      if (favorites.isEmpty)
        ...
        _FavoritesList(favorites: favorites, rates: state.rates),
    ]);
  }
}

// ── PRO баннер ────────────────────────────────────────────────────────────────
class _PaywallBanner extends ConsumerWidget {
  @override
  Widget build(BuildContext context, WidgetRef ref) {
        ...
        child: Row(children: [
          Expanded(
            child: Column(
              ...
              const Text('Flux PRO', style: TextStyle(
                  color: Colors.white, fontSize: 16,
                  fontWeight: FontWeight.w700)),
              const Gap(3),
              Text('Offline · No ads · Favorites · Full analytics',
          ...
          Container(
            ...
            child: const Text('\$1.99 / mo', style: TextStyle(
                color: Color(0xFF3d2fa0), fontSize: 13,
                fontWeight: FontWeight.w700)),
          ),
        ]),
      ),
...
class _AddCurrentPair extends ConsumerWidget {
  final Rate from, to;
  final bool isPro;
  const _AddCurrentPair(
      {required this.from, required this.to, required this.isPro});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isFav = ref.watch(
        favoritesProvider.select((list) => list.contains(pair)));

    return GestureDetector(
      onTap: () async {
        if (!isPro) {
          // Показываем paywall — если купил, разблокируем сохранение
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          const Gap(6),
          Text(
            isFav ? s.saved : (isPro ? s.savePair : 'PRO'),
            style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500,
                color: isFav ? AppTheme.accent
                    : isPro ? c.textSecondary
                    : AppTheme.accent),
          ),
        ]),
      ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const Gap(12),
        Text(s.noFavorites, style: TextStyle(fontSize: 15,
            fontWeight: FontWeight.w600, color: c.textPrimary)),
        const Gap(6),
        Text(s.noFavoritesHint, textAlign: TextAlign.center,
...
class _FavoritesList extends ConsumerWidget {
  ...
  const _FavoritesList(
      {required this.favorites, required this.rates});
          ...
          return Column(children: [
            Padding(
              ...
              child: Row(children: [
                ...
                const Spacer(),
                Text(_fmtRate(rate), style: TextStyle(
